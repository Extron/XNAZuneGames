...
{
    public class AlbumsMenu : MenuType
    {
        ...
        public void setDisplayList(string fontAsset, string highlighterAsset, Rectangle boudingBox, Color color, Vector2 highlighterLocation, int optionFactor, ITransition intro, ITransition revert, ITransition select, ITransition exit)
        {
            ...
            {
                OptionType option = new OptionType(albumCollection[i].Name, new Vector2(boudingBox.X, boudingBox.Y + optionFactor * i), 
        ...
        {
            MediaComponent.collectionSongs.setSongs(albumCollection[args.index].Name, albumCollection[args.index].Songs);
        }
