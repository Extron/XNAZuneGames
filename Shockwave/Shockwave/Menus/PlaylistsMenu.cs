...
{
    public class PlaylistsMenu : MenuType
    {
        ...
        {
            playlistCollection = library.Playlists;
        }
        ...
        public void setDisplayList(string fontAsset, string highlighterAsset, Rectangle boudingBox, Color color, Vector2 highlighterLocation, int optionFactor, ITransition intro, ITransition revert, ITransition select, ITransition exit)
        {
            ...
            {
                OptionType option = new OptionType(playlistCollection[i].Name, new Vector2(boudingBox.X, boudingBox.Y + optionFactor * i),
        ...
        {
            MediaComponent.collectionSongs.setSongs(playlistCollection[args.index].Name, playlistCollection[args.index].Songs);
        }
