using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using Microsoft.Xna.Framework.Graphics;
using Microsoft.Xna.Framework.Content;
using Reflex;
using Reverb.Delegates;
using Reverb.Components;
using Reverb.Components.Options;
using Reverb.Arguments;
using Reverb.Transitions;

namespace Reverb.Screens
{
    public class MenuType
    {
        #region Variables
        /// <summary>
        /// This event handler is raised when the user presses the back button.
        /// </summary>
        public Back Back;

        /// <summary>
        /// A dictionary of the menu's various components, accessible by a standardized unique string identification.
        /// </summary>
        public Dictionary<string, ComponentType> components;

        /// <summary>
        /// The current state of the menu's transitions.  Possible values are intro, exit, and select transitions, as well as 
        /// an idle menu state and an exiting final state.
        /// </summary>
        TransitionState transitionState;

        /// <summary>
        /// A unique string identifier for the menu to be accessed in a dictionary
        /// </summary>
        string state;

        /// <summary>
        /// A value that determines of the menu's transitions are queuable or listable.  Queuable transitions execute one at a time,
        /// in the order that they are sent to the transition manager.  No overlapping takes place with these transitions.  Listable
        /// transitions execute at the same time, and any that are sent to the manager are executed.  The transitions overlap, which
        /// is useful for designing fade and scroll transitions.
        /// </summary>
        bool queue;

        /// <summary>
        /// Determines whether this menu activates the intro transition of the previous menu when the back event is raised.
        /// </summary>
        bool activateIntro;

        /// <summary>
        /// Determines whether this menu activates its exit transition when the back event is raised.
        /// </summary>
        bool activateExit;

        /// <summary>
        /// Determines whether the previous menu is visible beneath the current menu.
        /// </summary>
        bool isOverlap;
        #endregion

        #region Properties
        /// <summary>
        /// Gets or sets the menu's current transition state.
        /// </summary>
        public TransitionState TransitionState
        {
            get { return transitionState; }
            set { transitionState = value; }
        }

        /// <summary>
        /// Determines if any of the menu's components are registered with intro transitions. 
        /// </summary>
        public bool HasIntro
        {
            get
            {
                bool temp = false;

                foreach (ComponentType component in components.Values)
                {
                    if (component.HasIntro)
                    {
                        temp = true;
                        break;
                    }
                }

                return temp;
            }
        }

        /// <summary>
        /// Determines if any of the menu's components are registered with revert transitions. 
        /// </summary>
        public bool HasRevert
        {
            get
            {
                bool temp = false;

                foreach (ComponentType component in components.Values)
                {
                    if (component.HasRevert)
                    {
                        temp = true;
                        break;
                    }
                }

                return temp;
            }
        }

        /// <summary>
        /// Determines if any of the menu's components are registered with select transitions. 
        /// </summary>
        public bool HasSelected
        {
            get
            {
                bool temp = false;

                foreach (ComponentType component in components.Values)
                {
                    if (component.HasSelect)
                    {
                        temp = true;
                        break;
                    }
                }

                return temp;
            }
        }

        /// <summary>
        /// Determines if any of the menu's components are registered exit intro transitions. 
        /// </summary>
        public bool HasExit
        {
            get
            {
                bool temp = false;

                foreach (ComponentType component in components.Values)
                {
                    if (component.HasExit)
                    {
                        temp = true;
                        break;
                    }
                }

                return temp;
            }
        }

        /// <summary>
        /// Checks to see if all of the menu's components have completed their transitions.
        /// </summary>
        public bool CompletedTransitions
        {
            get
            {
                bool temp = false;

                foreach (ComponentType component in components.Values)
                {
                    if (component.completedTransitions(transitionState))
                    {
                        temp = true;
                        continue;
                    }
                    else
                    {
                        temp = false;
                        break;
                    }
                }

                return temp;
            }
        }

        /// <summary>
        /// Gets or sets the menu's unique identifier.
        /// </summary>
        public string State
        {
            get { return state; }
            set { state = value; }
        }

        /// <summary>
        /// Gets or sets the queue variable, which determines of the menu's transitions are queuable or listable.
        /// </summary>
        public bool Queue
        {
            get { return queue; }
            set { queue = value; }
        }

        public bool IsOverlap
        {
            get { return isOverlap; }
            set { isOverlap = value; }
        }
        #endregion

        #region Constructors
        public MenuType(string current)
        {
            components = new Dictionary<string, ComponentType>();
            state = current;
            transitionState = TransitionState.none;
            queue = true;
            activateIntro = true;
            activateExit = true;
            isOverlap = false;
        }

        public MenuType(string identifier, bool queuable, bool intro, bool exit, bool overlap)
            : this(identifier)
        {
            queue = queuable;
            activateIntro = intro;
            activateExit = exit;
            isOverlap = overlap;
        }
        #endregion

        #region Overridable Functions
        /// <summary>
        /// Initializes all menu components at the menu's startup.
        /// </summary>
        public virtual void initialize()
        {
            foreach (ComponentType component in components.Values)
                component.initialize();
        }

        /// <summary>
        /// Loads all graphical data for the components at the menu's startup.
        /// </summary>
        public virtual void load(ContentManager content)
        {
            foreach (ComponentType component in components.Values)
                component.load(content);
        }

        /// <summary>
        /// Updates all of the menu's components, as well as checking if the user has pressed the back button.
        /// </summary>
        /// <param name="i">The game's menu input handler.</param>
        public virtual void update(InputHandler input)
        {
            foreach (ComponentType component in components.Values)
                component.update(input);

            back(input);
        }

        /// <summary>
        /// Draws all the menu's components.
        /// </summary>
        /// <param name="spriteBatch">The game's menu sprite batch.</param>
        public virtual void draw(SpriteBatch spriteBatch)
        {
            foreach (ComponentType component in components.Values)
                component.draw(spriteBatch);
        }

        /// <summary>
        /// Dynamically initializes specific items when the menu is accessed within the game.  This menu can be overrided within
        /// the game's menu to initialize components that require dynamic information from the game.
        /// </summary>
        public virtual void dynamicInitialize()
        {
        }

        public void setTransitions(TransitionState transition)
        {
            foreach (ComponentType component in components.Values)
                component.setTransition(transition);
        }

        /// <summary>
        /// Checks to see if the user has pressed the back button, and if so, raises the Back event and resets the menu data.
        /// </summary>
        /// <param name="i">The game's menu input handler.</param>
        public virtual void back(InputHandler i)
        {
            if (i.backActivated(false))
            {
                if (Back != null)
                    Back(new MenuArgs(state, activateIntro, activateExit));

                reset();
            }
        }

        /// <summary>
        /// Resets all of the menu components to their default values.
        /// </summary>
        public virtual void reset()
        {
            foreach (ComponentType component in components.Values)
                component.reset();
        }
        #endregion

        #region Menu Options
        /// <summary>
        /// Checks to see if any of the menu components are option components, and adds the event at the specified option.
        /// </summary>
        /// <param name="method">The event that needs to be added.</param>
        /// <param name="optionIndex">The option that needs the event.</param>
        public void setEvent(Select method, int optionIndex)
        {
            foreach (object component in components.Values)
            {
                IOptions options = component as IOptions;

                if (options != null)
                    options.setEvent(method, optionIndex);
            }
        }

        /// <summary>
        /// Checks to see if any of the menu components are option components, and adds the event to all options.
        /// </summary>
        /// <param name="method">The event that needs to be added.</param>
        public void setEvents(Select method)
        {
            foreach (object component in components.Values)
            {
                IOptions options = component as IOptions;

                if (options != null)
                    options.setEvents(method);
            }
        }

        /// <summary>
        /// Initializes all of the components' transitions, storing default values.
        /// </summary>
        public void initializeTransitions()
        {
            foreach (ComponentType component in components.Values)
                component.initializeTransitions();
        }

        /// <summary>
        /// Updates all of the components' transitions.
        /// </summary>
        public void updateTransitions()
        {
            foreach (ComponentType component in components.Values)
                component.updateTransitions(transitionState);
        }

        /// <summary>
        /// Determines if the menu's components have any stored transition for the desired state.
        /// </summary>
        /// <param name="state">The transition state to check for transitions.</param>
        /// <returns>Returns true if at least one of the components has a transition, otherwise returns false.</returns>
        public bool hasTransition(TransitionState state)
        {
            bool returnValue = false;

            switch (state)
            {
                case TransitionState.intro:
                    returnValue = HasIntro;
                    break;

                case TransitionState.revert:
                    returnValue = HasRevert;
                    break;

                case TransitionState.select:
                    returnValue = HasSelected;
                    break;

                case TransitionState.exit:
                    returnValue = HasExit;
                    break;
            }

            return returnValue;
        }

        /// <summary>
        /// Adds an IComponent implemented component to the menu's list of components.
        /// </summary>
        /// <param name="component">The component to be added.</param>
        protected void addComponent(ComponentType component)
        {
            components.Add(component.Identifier, component);
        }
        #endregion
    }
}
